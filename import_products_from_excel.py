...
def get_or_create_category(name):
    ...
    if not category:
        print(f"⚠️  Catégorie non trouvée: {name}")
        return None
...
def get_or_create_subcategory(name, category):
    ...
    if not subcategory:
        print(f"⚠️  Sous-catégorie non trouvée: {name} -> {normalized_name} dans {category.name}")
        return None
    ...
    for index, row in df.iterrows():
        try:
            ...
            if not reference or not name or not cat_name or not subcat_name:
                print(f"\n⚠️  Ligne {index + 2}: Données obligatoires manquantes - IGNORÉE")
                skipped_count += 1
                continue
            
            # Vérifier et nettoyer le prix et la quantité
            if pd.isna(price) or str(price).strip() in ['-', '', 'nan']:
                print(f"\n⚠️  Ligne {index + 2}: Prix invalide - IGNORÉE")
                skipped_count += 1
                continue
            
            if pd.isna(quantity) or str(quantity).strip() in ['-', '', 'nan']:
                print(f"\n⚠️  Ligne {index + 2}: Quantité invalide - IGNORÉE")
                skipped_count += 1
                continue
            ...
            try:
                ...
                print(f"\n⚠️  Ligne {index + 2}: Prix ou quantité invalide - IGNORÉE")
                skipped_count += 1
                continue
            ...
            if existing_product:
                print(f"   ℹ️  Produit existe déjà: {reference} - IGNORÉ")
                skipped_count += 1
                continue
            ...
            with transaction.atomic():
                ...
                # Ajouter brand seulement s'il existe
                if brand:
